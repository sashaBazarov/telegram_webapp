...
from app.database import add_product, find_product_by_id, remove_product, find_all_products, update_product, find_users_products
...
async def get_users_products(user_id: int):
    if not find_product_by_id(user_id):
...
async def edit_product(filter: FilterProduct, data: UpdateProduct):
    ...
    update_product(filter.id, data.name, data.description, data.price, data.image)
